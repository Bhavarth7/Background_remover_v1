...
# Model Components (unchanged from previous code)
class Mlp(nn.Module):
...
class BackgroundRemover:
    ...
    async def process_image(self, image: UploadFile) -> str:
        temp_path = Config.TEMP_DIR / self._sanitize_filename(image.filename)
        try:
            with temp_path.open("wb") as buffer:
                shutil.copyfileobj(image.file, buffer)
            pil_image = Image.open(temp_path)
            output = self.model.inference(pil_image)
            output_path = Config.RESULT_DIR / self._sanitize_filename(image.filename)
...
# Dependency to get BackgroundRemover instance
async def get_background_remover():
...
app = FastAPI(
    ...
    description="API for removing backgrounds from images using the BEN2 model.",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc"
)
...
@app.post(
    ...
    description="Upload an image file to remove its background. Returns a processed image with an alpha channel.",
    responses={
        200: {
        ...
        422: {
            "description": "Validation error (e.g., invalid image format)",
            "content": {"application/json": {"example": {"detail": "Invalid image format"}}}
        },
        500: {
            "description": "Internal server error",
            "content": {"application/json": {"example": {"detail": "Error processing image"}}}
        }
    }
)
async def remove_background(
    image: UploadFile = File(
        ...,
        description="Image file to process (JPEG, PNG, etc.). Must be RGB or convertible to RGB."
    ),
    ...
    - **Input Format**: Single image file (multipart/form-data).
    - **Supported Formats**: JPEG, PNG, BMP, etc. (any format PIL can open).
    - **Output**: PNG image with alpha channel (transparent background).
    """
    # Validate file type
    ...
    try:
        # Process the image
        output_path = await remover.process_image(image)
        return FileResponse(
            output_path,
            media_type="image/png",
            filename=Path(output_path).name,
            headers={"message": "Background removed successfully"}
        )
